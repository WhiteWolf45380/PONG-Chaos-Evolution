...
class Modifiers(pm.states.State):
    ...
    def __init__(self):
        ...
        self.add("border_color_ennemy", (230, 59, 48), category="paddle", add_prefix=True)              # (color): couleur de la bordure d'un ennemi
    ...
    def add(
        ...
        sessions: Optional[str | Iterable[str]] = None,
        modes: Optional[str | Iterable[str]] = None,
        ...
        Args:
            ...
            sessions  (str | Iterable[str], optional): sessions exclusives (vide = toutes)
            modes     (str | Iterable[str], optional): modes exclusifs (vide = tous)
            add_prefix (bool, optional)            : ajoute automatiquement un préfixe selon la catégorie
            to_save   (bool, optional)             : sauvegarde du paramètre dans un fichier externe
        """
    ...
    def get(self, name: str, index: Optional[int] = None, fallback: Any = None):
        ...
        Args:
            name     (str)        : nom du paramètre
            index    (int | None) : indice de l'élément (si le paramètre est itérable)
            fallback (Any)        : valeur de secours si le paramètre est absent

        Returns:
            Valeur du paramètre, ou l'élément à l'index si spécifié, ou fallback
        """
    ...
    def get_with_filters(
        ...
        """
        Renvoie tous les paramètres se conformant aux filtres fournis.
        Un paramètre sans restriction de session/mode est toujours inclus.

        Args:
            category      (str | None) : filtre par catégorie
            session       (str | None) : filtre par session
            mode          (str | None) : filtre par mode de jeu
            remove_prefix (bool)       : retire le préfixe de catégorie des clés
        ...
        for name, param in self.params.items():
            ...
            # Filtre par session (liste vide = aucune restriction)
            if session is not None and param["sessions"] and session not in param["sessions"]:
                continue

            # Filtre par mode (liste vide = aucune restriction)
            if mode is not None and param["modes"] and mode not in param["modes"]:
                continue
    ...
    def get_by_category(self, category: Optional[str], remove_prefix: bool = False) -> dict:
        """
    ...
    def get_by_session(self, session: str) -> dict:
        """
        Renvoie tous les paramètres disponibles dans une session donnée.
        Les paramètres sans restriction de session sont toujours inclus.
        ...
        return {
            name: param["value"]
            for name, param in self.params.items()
            if not param["sessions"] or session in param["sessions"]
        }

    def get_by_mode(self, mode: str) -> dict:
        """
        Renvoie tous les paramètres disponibles dans un mode de jeu donné.
        Les paramètres sans restriction de mode sont toujours inclus.
        ...
        return {
            name: param["value"]
            for name, param in self.params.items()
            if not param["modes"] or mode in param["modes"]
        }

    def get_categories(self) -> list[str]:
        """
        Renvoie la liste de toutes les catégories enregistrées
    ...
    def set(self, name: str, value: object, index: Optional[int] = None):
        ...
        Args:
            ...
            index (int | None) : indice de l'élément à modifier (si le paramètre est itérable)
        """
        if name not in self.params:
    ...
    # ======================================== GESTION DE DONNEES ========================================
    def load(self):
        """Charge les paramètres sauvegardés depuis le fichier de données"""
        try:
            data = pm.data.load(self.data_path)
            for k, v in data.items():
                self.params[k] = v
        except (FileNotFoundError, RuntimeError):
            print(f"[Data] No file found at {self.data_path}")

    def save(self):
        """Sauvegarde les paramètres marqués `to_save` dans le fichier de données"""
        try:
